...
//    
    tb.add('-');
//    
    tb.x_buttons.reset = tb.add({
      xtype: 'checkbox',
      boxLabel: basel('labelOverwrite'),
      id: 'file_upload_overwrite',
      value: 1,
      //tooltip: this.i18n.reset_btn_tip,
      //iconCls: 'ext-ux-uploaddialog-resetbtn',
      //handler: this.onResetButtonClick,
...
//
    tb.add('-');
//
    tb.x_buttons.type = tb.add(this.fileUploadTypeCombo( this.grid_panel ));
//    
    tb.add('-');
//    
...
//    
  ...
  fileUploadTypeStore : function() {
    var store = new Ext.data.JsonStore({
		proxy: new Ext.data.HttpProxy({
			url: '/cgi-bin/wg/modules/site/item.cgi'
		}),
		baseParams: {
			action: 'site_item_file_type'
		},
		fields: ['id', 'alias']
	});
  ...
  fileUploadTypeCombo : function( gp )  {
    if( Ext.getCmp( 'file_upload_type') )  {
      return Ext.getCmp('file_upload_type');
    }
    else  {
      var b = new Ext.form.ComboBox({
        xtype: 'combo',
        width: 150,
        store: this.fileUploadTypeStore(),
        name: 'type_id_old',
        hiddenName: 'type_id',
        id: 'file_upload_type',
        displayField: 'alias',
        ...
        triggerAction: 'all',
        //mode: 'local',
        emptyText: basel('labelSelectFileType'),
        selectOnFocus: true,
        scope: this,
        listeners: {
          'select' : function() {
            var tb = gp.getBottomToolbar();
...
//        .dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
      ...
      if (Ext.getCmp('file_upload_type').getValue() > 0 ) {
        tb.x_buttons.upload.enable();
      }
      else {
        tb.x_buttons.upload.disable();
...
p.i18n = {
  ...
  note_col_title: 'Note',
  note_col_width: 150,
  add_btn_text: 'Adicionar',
  add_btn_tip: 'Adicionar imagem.',
  ...
  upload_btn_start_tip: 'Upload.',
  upload_btn_stop_tip: 'Parar upload.',
  close_btn_text: 'Fechar',
  close_btn_tip: 'Fechar',
  progress_waiting_text: 'Aguarde...',
  progress_uploading_text: 'Imagens: {0} de {1} imagens.',
