package HellFire::User::ValueDAO;
...
sub load {
	...
	if( ref $obj[0] eq 'HellFire::User::FieldDAO' && ref $obj[1] eq 'HellFire::User::ItemDAO' ) {
		if( $obj[0]->get_type_id() && $obj[1]->get_id() && $obj[0]->get_id() ) {
			my $sth = $self->dbh->prepare( 'call user_value_load(?,?,?,?,@c,@w);' );
...
sub add {
	...
	if( $self->get_parent_id() && $self->get_item_id() && $type ) {
		...
		my $query = "insert into user_values_$type( parent_id, item_id, value ) values(?,?,?)";
...
sub destroy {
	my $self = shift;
	my $type = $self->get_type();
warn $self->get_parent_id();
warn $self->get_item_id();
warn $type;
	if( $self->get_parent_id() && $self->get_item_id() && $type ) {
		my $query = "delete from user_values_$type where item_id = ? and parent_id = ? ";
