package HellFire::Settings::ValueDAO;
...
sub load {
	...
	if( ref $obj[0] eq 'HellFire::Settings::FieldDAO' && ref $obj[1] eq 'HellFire::Settings::ItemDAO' ) {
		if( $obj[0]->get_type_id() && $obj[1]->get_id() && $obj[0]->get_id() ) {
			my $sth = $self->dbh->prepare( 'call settings_value_load(?,?,?,?,@c,@w);' );
...
sub add {
	...
	if( $self->get_parent_id() && $self->get_item_id() && $type ) {
		...
		my $query = "insert into settings_values_$type( parent_id, item_id, value ) values(?,?,?)";
...
sub destroy {
	...
	if( $self->get_parent_id() && $self->get_item_id() && $type ) {
		my $query = "delete from settings_values_$type where item_id = ? and parent_id = ? ";
