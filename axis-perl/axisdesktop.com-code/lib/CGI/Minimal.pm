...
sub cookie {
	...
	CGI::Util_M::rearrange(['NAME',['VALUE','VALUES'],'PATH','DOMAIN','SECURE','EXPIRES','HTTPONLY'],@p);

    # if no value is supplied, then we retrieve the
    # value of the cookie, if any.  For efficiency, we cache the parsed
    # cookies in our state variables.
    unless ( defined($value) ) {
	$self->{'.cookies'} = CGI::Cookie_M->fetch
	    unless $self->{'.cookies'};
...
sub param_fileupload { require CGI::Minimal::Multipart; &_internal_param_fileupload(@_);  }
